/**
 * Login Page — OpsPulse Theme
 * OTP-based authentication with 2 steps:
 * 1. Enter email → sends OTP via backend POST /auth/send-otp
...
import { Mail, KeyRound, ArrowLeft, Loader2, Activity } from 'lucide-react'
...
const Login = () => {
  ...
  const handleOtpChange = (e) => {
    setOtp(e.target.value.replace(/\D/g, '').slice(0, 6)) // Only digits, max 6
    if (error) clearAuthError()
    if (formError) setFormError(null)
  }
      ...
      <div className="w-full max-w-[420px] relative z-10">
        ...
        <div className="bg-[#0b0d18] border border-white/[0.07] rounded-xl p-8">
          {step === 1 ? (
            ...
            <form onSubmit={handleSendOTP} className="space-y-5">
              ...
              {/* Email Field */}
              <div>
                <label htmlFor="login-email" className="block font-barlow text-[9px] tracking-[0.2em] uppercase text-brand-offwhite/40 mb-2.5">
                  Email Address
                </label>
            ...
            <form onSubmit={handleVerifyOTP} className="space-y-5">
              ...
              <div className="p-3 bg-white/[0.03] border border-white/[0.07] rounded-lg">
                <span className="font-barlow text-[9px] tracking-[0.16em] uppercase text-brand-offwhite/30">Sending to</span>
                <div className="font-barlow text-[12px] tracking-[0.06em] text-brand-offwhite mt-0.5">{email}</div>
              </div>

              {/* OTP Field */}
              <div>
                <label htmlFor="login-otp" className="block font-barlow text-[9px] tracking-[0.2em] uppercase text-brand-offwhite/40 mb-2.5">
                  Enter 6-Digit Code
                </label>
                <div className="relative">
        ...
        <div className="mt-8 text-center">
          <p className="font-barlow text-[10px] tracking-[0.14em] uppercase text-brand-offwhite/30">
            ...
            <Link to="/signup" className="text-brand-offwhite/60 hover:text-brand-offwhite transition-colors">
              Sign up here
            </Link>
          </p>
        </div>
